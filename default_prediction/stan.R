...
model <- '
  ...
  model {
    for (i in 1:n) {
      y[i] ~ bernoulli(inv_logit(b0+b[1]*x1[i]+b[2]*x2[i]+b[3]*x3[i]+b[4]*x4[i]+b[5]*x5[i]));
      x1[i] ~ normal(mu,s);
...
init_fun <- function(chain_id) {
  ...
  return(list(mu=mu,s=s,b0=b0,q=q,lambda1=lambda1,lambda2=lambda2,lambda3=lambda3,lambda=lambda,b=b))
}
